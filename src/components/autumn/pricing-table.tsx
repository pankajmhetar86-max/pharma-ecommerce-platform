...
export default function PricingTable({ productDetails }: { productDetails?: Array<ProductDetails> }) {
  ...
  const intervalFilter = (product: Product) => {
    ...
    if (multiInterval) {
      if (isAnnual) {
        return intervalGroup === 'year'
      } else {
        return intervalGroup === 'month'
      }
    }
    ...
    <div className={cn('root')}>
      ...
      >
        {products
          ...
          .filter((product) => !product.properties.is_one_off)
          .map((product, index) => (
            <PricingCard
              key={index}
...
export const usePricingTableContext = (_componentName: string) => {
...
export const PricingCard = ({ productId, className, buttonProps }: PricingCardProps) => {
  ...
  const mainPriceDisplay = product.properties.is_free
    ? { primary_text: 'Free' as const, secondary_text: undefined as string | undefined }
    : (product.items[0]?.display ?? { primary_text: '—', secondary_text: undefined as string | undefined })
      ...
      <div className={cn('flex flex-col h-full flex-grow', isRecommended && 'lg:translate-y-6')}>
        <div className="h-full">
          <div className="flex flex-col">
            <div className="pb-4">
              <h2 className="text-2xl font-semibold px-6 truncate">{(productDisplay ?? product).name}</h2>
...
export const RecommendedBadge = ({ recommended }: { recommended: string }) => {
  return (
    <div className="bg-secondary absolute border text-muted-foreground text-sm font-medium lg:rounded-full px-3 lg:py-0.5 lg:top-4 lg:end-4 top-[-1px] end-[-1px] rounded-es-lg">
      {recommended}
    </div>
  )
}
